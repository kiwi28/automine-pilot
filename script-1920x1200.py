...
def perform_actions():
    ...
    # Click location A
    pyautogui.click(x=58, y=119)  # end session
    time.sleep(1)  
    
    # Click location B
    pyautogui.click(x=23, y=43)  # go back
    time.sleep(1)  
    
    pyautogui.moveTo(500, 500)  # Move mouse to a neutral location, so the scroll works
    time.sleep(1)  
    # Scroll down 20 lines
    pyautogui.scroll(-2000)  # Negative number scrolls down
    time.sleep(1)  # Wait for scroll to complete
    
    # Click location C
    pyautogui.click(x=963, y=1109)  # start new session
    time.sleep(1)  
    pyautogui.hotkey('alt', 'tab')

def main():
    # setup to get button positions on screen --------------------------------- run this once to get the positions of the buttons
